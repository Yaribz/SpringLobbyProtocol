...
our %SERVER_CMD_NB_PARAMS=(
  ...
  ENDOFCHANNELS => 0,
  CHANNELTOPIC => [2,4], # legacy clients expect timestamp
  CLIENTS => 2,
  JOINED => 2,
...
sub marshallClientCommand { marshallCommand(\%CLIENT_CMD_SENTENCE_POS,\%CLIENT_CMD_NB_PARAMS,@_) }
sub marshallServerCommand { marshallCommand(\%SERVER_CMD_SENTENCE_POS,\%SERVER_CMD_NB_PARAMS,@_) }
...
sub marshallCommand {
  my ($r_sentencePos,$r_nbParams,$r_u,$cmdId)=@_;
...
sub unmarshallCommand {
  ...
  my ($sentencePos,$addSentences);
  if(defined $r_sentencePos->{$cmd}) {
    ($sentencePos,$addSentences)=@{$r_sentencePos->{$cmd}};
    $sentencePos=3 if($sentencePos == -1); # special case for CHANNELTOPIC command
  }
  my $r_u;
  my $nbParams=$r_nbParams->{$cmd};
  if($sentencePos) {
    ...
    if(@words == $sentencePos+1) {
      ...
      if($addSentences) {
        my $sentencesString=pop(@words);
        my @sentences = $sentencesString eq '' ? ('') : split(/\t/,$sentencesString,$addSentences == -1 ? -1 : $addSentences+1);
    ...
    $r_u=[map {s/ / /gr} split(/ /, $m, defined $nbParams ? -1 : 2)];
  }
  if(defined $nbParams) {
    my $errorMsg="invalid number of parameters for $cmd command: got $#{$r_u}, expected";
    if(ref $nbParams) {
